...
type props = {
    student: student,
    positiveAnnotations: annotation[],
    allSessions: session[]
}

const Positives: NextPage<props> = ({student, positiveAnnotations, allSessions}) => {
        ...
        <div className="min-h-screen flex flex-col justify-between space-y-10">
            ...
            <main className="flex flex-col items-center justify-center">
                <div className="max-w-lg md:max-w-2xl lg:max-w-4xl w-full flex flex-col items-center justify-center">
                    <div className="w-full text-4xl text-blue-400 border-b border-zinc-500 px-4 text-center md:text-left md:mx-4 py-2 flex flex-col items-left justify-center">
                        ...
                        <div className="flex flex-row items-center justify-start space-x-3">
                            <h1>Positive Anmerkungen zu {student.name}</h1>
                        </div>
                ...
                <div className="max-w-2xl w-full flex flex-col items-center justify-center space-y-2 mt-10">
                    {
                        positiveAnnotations.map(annotation => {
                    ...
                    {positiveAnnotations.length == 0 && (
                        <p>{student.name} hat keine positiven Anmerkungen erhalten.</p>
                    )}
                </div>
...
export const getServerSideProps: GetServerSideProps = async (context) => {
    ...
    const positiveAnnotations = await prisma.annotation.findMany({
        where: {
            student: {
                id: studentId
            },
    ...
    return {
        props: {student, positiveAnnotations, allSessions}
    }
}
